...
cidrs18binnedfreq <- melt(cidrs18, id.vars = c("SubjID","Condition","Blue1Red2New0"), measure.vars = c("SourceConf"))
cidrs18binnedfreq2 <- dcast(cidrs18binnedfreq, Condition + SubjID + Blue1Red2New0 ~ value, length)

cidrs18Item<-cidrs18binnedfreq2[cidrs18binnedfreq2$Condition==1,]

cidrs18ItemS_info <- list(
  numberParticipants = length(unique(cidrs18Item$SubjID)),
  testData_signalitems = as.matrix(cidrs18Item[cidrs18Item$Blue1Red2New0=="2",as.character(1:6)]),
  testData_noiseitems = as.matrix(cidrs18Item[cidrs18Item$Blue1Red2New0=="1",as.character(1:6)])
)
...
cidrs18ContextS_info <- list(
  numberParticipants = length(unique(cidrs18Context$SubjID)),
  testData_signalitems = as.matrix(cidrs18Context[cidrs18Context$Blue1Red2New0=="2",as.character(1:6)]),
  testData_noiseitems = as.matrix(cidrs18Context[cidrs18Context$Blue1Red2New0=="1",as.character(1:6)])
)
